...
; The jugs are irregularly shaped and unmarked; no water can be spilled; 
; each step pouring water from one jug to another stops when 
; either the source jug is empty or the destination jug is full, 
; whichever happens first. 
; Use Z3 to find a solution (and present the answer in human readable form)
;
; Hints:
; 1. Proposal for a model:
;   (declare-fun C (Int Int) Int)
; where (C j t) represents the contents of jug number j at step t
; 
; 2. The data from the puzzle allows you to conclude that 
; a) the total amount of water remains constant, and 
; b) at each step, one of the jugs remains unaltered
	...
	(implies 
		...
		(exists ((j1 Int) (j2 Int) (j3 Int))    ; this way you don't have to work with a 'or' and all combinations explicitly written down...
			(and
				...
				(= (C t j1)(C (+ t 1) j1))  ; one jug remains as it is
